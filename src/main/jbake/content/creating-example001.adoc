...
`DukesAgeResource`, a _Jakarta RESTful Web Service_; `DukesBirthdayBean`,
...
`DukesAgeResource` is a _Jakarta RESTful Web Service_ resource that calculates the age of Duke,
...
the user's birth date, uses the _Jakarta RESTful Web Service_ Client API to get Duke's current
...
The `DukesAgeResource` web service is a _Jakarta RESTful Web Service_ resource. The
...
CDI managed bean uses the _Jakarta RESTful Web Service_ client API to access the
